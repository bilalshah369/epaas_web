...
import { fetchCEOPending, fetchCEOAll, fetchCEOAppeals, type Appeal } from '@/services/ceo.service';
...
function exportAppealsCSV(rows: Appeal[], filename: string) {
  if (!rows.length) return;
...
const NOTIFICATIONS = [
  { id: 1, title: 'New application for CEO review — EPAAS-2025-007', desc: 'Forwarded from Nodal Point B. Awaiting CEO decision.', time: '1h ago', read: false },
  { id: 2, title: 'Appeal filed — EPAAS-2025-003', desc: 'Applicant has filed an appeal against rejection decision.', time: '3h ago', read: false },
  { id: 3, title: 'Pending decision — EPAAS-2025-005', desc: 'Application has been pending with CEO for 12 days.', time: '1d ago', read: true },
];
...
export default function CEODashboard() {
  ...
  const [pendingSubTab, setPendingSubTab] = useState<'appeals' | 'decisions'>('appeals');
  const [apps, setApps]               = useState<Application[]>([]);
  const [allApps, setAllApps]         = useState<Application[]>([]);
  const [appeals, setAppeals]         = useState<Appeal[]>([]);
  const [loading, setLoading]         = useState(true);

  // filter state for work queue
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([fetchCEOPending(), fetchCEOAll(), fetchCEOAppeals()])
      .then(([pending, all, apls]) => {
        setApps(pending); setPendingDisplay(pending); setAllApps(all); setAppeals(apls);
      })
      .catch(() => toast.error('Failed to load CEO data'))
            ...
            {loading ? (
              ...
              return (
                <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg }}>
                  ...
                  <td style={S.td}>
                    <button onClick={() => navigate(`/ceo/appeals/${a.id}`)}
  ...
  const renderDashboard = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
        {[
          { icon: '⏳', label: 'Pending with CEO',   count: apps.length,    color: '#B45309'      },
          { icon: '✅', label: 'Approved',           count: approvedCount,  color: COLORS.success  },
          { icon: '❌', label: 'Rejected',           count: rejectedCount,  color: COLORS.danger   },
          { icon: '⚖️', label: 'Pending Appeals',   count: appeals.filter((a) => a.status === 'AppealPending').length, color: COLORS.info },
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.6 }}>CEO Work Queue</span>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => exportCSV(apps, 'ceo_pending')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
        ...
        {loading ? (
          ...
          <div style={{ padding: '48px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, color: COLORS.textMuted }}>No applications pending CEO review.</div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                { label: 'Pending Appeals',                     value: appeals.filter((a) => a.status === 'AppealPending').length },
              ].map((item, i) => (
          ...
          <div style={{ background: COLORS.primaryLight, border: `1px solid ${COLORS.primary}22`, borderRadius: 8, padding: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 160 }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: COLORS.primary, fontWeight: 600 }}>CEO Decision Authority</div>
              <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 8, lineHeight: 1.6 }}>Forward to Chairperson or decide on appeals and applications</div>
            </div>
          </div>
        </div>
      </div>
  ...
  const renderPendingActions = () => (
    <>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${COLORS.border}`, marginBottom: 14 }}>
        {(['appeals', 'decisions'] as const).map((t) => (
          <div key={t} onClick={() => setPendingSubTab(t)}
            ...
            {t === 'appeals' ? 'Pending Appeals' : 'Pending Decisions'}
          </div>
        ))}
      </div>

      {pendingSubTab === 'appeals' && (
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.6 }}>Pending Appeals ({appeals.length} records)</span>
            <button onClick={() => exportAppealsCSV(appeals, 'ceo_appeals')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['Sr.', 'App No.', 'Company', 'Product', 'Appeal Grounds', 'Filed On', 'Status', 'Action'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={8} style={{ ...S.td, textAlign: 'center', padding: '40px 0', color: COLORS.textMuted }}>Loading…</td></tr>
                ) : appeals.length === 0 ? (
                  <tr><td colSpan={8} style={{ ...S.td, textAlign: 'center', padding: '40px 0', color: COLORS.textMuted }}>No appeals pending.</td></tr>
                ) : appeals.map((a, i) => (
                  <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg }}>
                    ...
                    <td style={{ ...S.td, maxWidth: 110, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.application?.productName ?? '—'}</td>
                    <td style={{ ...S.td, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={a.grounds}>{a.grounds?.slice(0, 60)}{(a.grounds?.length ?? 0) > 60 ? '…' : ''}</td>
                    <td style={S.td}>{fmtDate(a.filedAt)}</td>
                    <td style={S.td}><StatusBadge status={a.status} label={a.status.replace('Appeal', '')} /></td>
                    <td style={S.td}>
                      <button onClick={() => navigate(`/ceo/appeals/${a.application?.id ?? a.applicationId}`)}
                        style={{ background: COLORS.primary, color: '#fff', border: 'none', borderRadius: 5, padding: '4px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                        Review Appeal
                      </button>
                    </td>
        ...
        <>
          ...
          <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
              ...
              <button onClick={() => exportCSV(pendingDisplay, 'ceo_pending_decisions')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
            </div>
