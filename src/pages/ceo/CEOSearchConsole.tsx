...
const STAGE_PENDING_WITH: Record<string, string> = {
  ...
  AppealPending: 'CEO', AppealApproved: '—', AppealRejected: '—',
...
const STAGE_OPTIONS: Record<string, string> = {
  'All Stages':                  '',
  'With CEO':                    'WithCEO',
  'Forwarded to Chairperson':    'WithChairperson',
  'Appeal Pending':              'AppealPending',
  'Appeal Approved':             'AppealApproved',
  'Appeal Rejected':             'AppealRejected',
  'Approved':                    'Approved',
  'Rejected':                    'Rejected',
};
...
const SUMMARY_LABELS = [
  ...
  { label: 'Appeals on File',        fn: (a: Application) => ['AppealPending', 'AppealApproved', 'AppealRejected'].includes(a.stage) },
];

export default function CEOSearchConsole() {
      ...
      {searched && (
        <div style={{ background: '#fff', border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={() => exportCSV(results, 'ceo_search_results')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
                ...
                {results.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }}
                      onClick={() => navigate(`/ceo/appeals/${a.id}`)}>
