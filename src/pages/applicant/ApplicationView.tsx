...
import {
  ...
  sendCertificateEmail, submitPmsReport,
  type Application, type AppFormData, type Query,
} from '@/services/application.service';
import { fetchExtensions, type ExtensionItem } from '@/services/extension.service';
...
// ── Tab: Queries ──────────────────────────────────────────────────────────────
function TabQueries({ app, extensions, onResponded }: { app: Application; extensions: ExtensionItem[]; onResponded: () => void }) {
  ...
  useEffect(() => {
    fetchQueries(app.id)
      .then((qs) => {
        ...
        setQueries(qs.filter((q) => q.nodalForwardedAt !== null || q.originStage !== 'WithTechnicalOfficer'));
      })
          ...
          {q.response ? (
            <div>
              ...
              <div style={{ background: '#F0FDF4', border: `1px solid #BBF7D0`, borderRadius: 6, padding: '10px 12px', fontSize: 12, color: COLORS.text, lineHeight: 1.6 }}>
                {(() => {
                  const { body, attachmentFile, attachmentName } = parseResponse(q.response!);
                  return (
                    <>
                      <span style={{ whiteSpace: 'pre-wrap' }}>{body}</span>
                      {attachmentFile && (
                        <div style={{ marginTop: 8 }}>
                          <a href={`${API_BASE}/uploads/${attachmentFile}`} target="_blank" rel="noreferrer"
                            style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: COLORS.primaryLight, color: COLORS.primary, padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, textDecoration: 'none' }}>
                            📎 {attachmentName}
                          </a>
                        </div>
                      )}
            ...
            <div>
              ...
              {(() => {
                ...
                const approvedExt = extensions.find((e) => e.queryId === q.id && e.status === 'Approved');
                ...
                return (
                  <div style={{ marginBottom: 10, padding: '8px 12px', borderRadius: 7, background: isOverdue ? '#FEF2F2' : '#FFFBF0', border: `1px solid ${isOverdue ? '#FECACA' : 'rgba(246,173,85,0.5)'}`, fontSize: 11, color: isOverdue ? '#991B1B' : '#7C2D12' }}>
                    <span style={{ fontWeight: 700 }}>⏰ Response Deadline: </span>
                    <span style={{ fontWeight: 700 }}>
                      {deadline.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </span>
                    <span style={{ marginLeft: 6, color: isOverdue ? '#DC2626' : '#92400E' }}>
                      {isOverdue ? `(${Math.abs(daysLeft)} day(s) overdue)` : `(${daysLeft} day(s) remaining)`}
                    </span>
                    {approvedExt && (
                      <span style={{ marginLeft: 8, background: '#D1FAE5', color: '#065F46', padding: '1px 7px', borderRadius: 8, fontWeight: 700, fontSize: 10 }}>
...
interface Milestone { id: string; label: string; sublabel?: string }
...
const CORE_MILESTONES: Milestone[] = [
  ...
  { id: 'form2Prep',    label: 'Form 2 Preparation',    sublabel: 'Technical Officer' },
...
function getCurrentMilestoneId(app: Application): string {
  ...
  switch (app.stage) {
    ...
    case 'WithNodalOfficerA':    return hasEcDone ? 'decisionPrep' : 'nodalReview';
    case 'QuerySent':
    case 'WithTechnicalOfficer': return hasEcDone ? 'form2Prep'    : 'toReview';
    case 'WithExpertCommittee':  return 'ecReview';
    case 'DecisionPending':      return 'decisionPrep';
    case 'Approved': case 'Closed': return 'approved';
    case 'Rejected':             return 'rejected';
    case 'AppealPending':        return 'appealSubmitted';
    case 'WithCEO':              return 'ceoReview';
    case 'WithChairperson':      return 'chairReview';
    default:                     return 'nodalReview';
  }
}
...
function TabTimeline({ app, queries, extensions }: { app: Application; queries: Query[]; extensions: ExtensionItem[] }) {
  ...
  const onAppealPath = ['AppealPending', 'WithCEO', 'WithChairperson'].includes(app.stage);
  ...
  let deadlineInfo: { deadline: Date; extDays: number; extGranted: boolean } | null = null;
  if (app.stage === 'QuerySent') {
    const openQuery = queries
      .filter((q) => !q.response)
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
    if (openQuery) {
      const baseDate = new Date(openQuery.nodalForwardedAt ?? openQuery.createdAt);
      const approvedExt = extensions.find((e) => e.queryId === openQuery.id && e.status === 'Approved');
      const extDays = approvedExt ? approvedExt.extensionDays : 0;
      deadlineInfo = { deadline: addDays(baseDate, 30 + extDays), extDays, extGranted: !!approvedExt };
    }
  }
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      {milestones.map((m, i) => {
        ...
        const isOutcome = m.id === 'approved' || m.id === 'rejected' || m.id === 'outcome';
        const dotColor  = done || active ? (isRejected && isOutcome ? COLORS.danger : COLORS.primary) : COLORS.border;
          ...
          <div key={m.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 24, flexShrink: 0 }}>
              <div style={{
                ...
                background: active
                  ? (isRejected && isOutcome ? COLORS.danger : COLORS.primary)
                  : done ? COLORS.primaryLight : '#E5E7EB',
                border: `2px solid ${dotColor}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 9, color: done || active ? (isRejected && isOutcome ? '#fff' : COLORS.primary) : COLORS.textMuted, fontWeight: 700,
              }}>
                {done && !active ? '✓' : active && isRejected && isOutcome ? '✕' : ''}
              </div>
            ...
            <div style={{ paddingBottom: 20, opacity: future ? 0.4 : 1 }}>
              <div style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? (isRejected && isOutcome ? COLORS.danger : COLORS.primary) : COLORS.text }}>
                {m.label}
                {active && (
              ...
              {m.sublabel && (
                <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 1 }}>{m.sublabel}</div>
              )}
      ...
      {app.stage === 'QuerySent' && (
        <div style={{ marginTop: 8, background: COLORS.warningLight, border: '1px solid rgba(246,173,85,0.5)', borderRadius: 8, padding: '12px 16px', fontSize: 12, color: '#7C2D12' }}>
          <div style={{ fontWeight: 700 }}>⚠ Query Sent — Response Required</div>
          {deadlineInfo && (
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <div style={{ fontSize: 11 }}>
                <span style={{ fontWeight: 600 }}>Response Deadline: </span>
                <span style={{ fontWeight: 700, color: '#9A3412' }}>
                  {deadlineInfo.deadline.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                </span>
                <span style={{ color: '#92400E' }}> (30 days{deadlineInfo.extGranted ? ` + ${deadlineInfo.extDays} day extension` : ''})</span>
              </div>
              {deadlineInfo.extGranted && (
                <div style={{ fontSize: 11, background: '#D1FAE5', border: '1px solid #6EE7B7', borderRadius: 5, padding: '4px 8px', color: '#065F46', fontWeight: 600, display: 'inline-block', width: 'fit-content' }}>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  useEffect(() => {
    ...
    fetchQueries(id)
      .then((qs) => setTimelineQueries(qs.filter((q) => q.nodalForwardedAt !== null || q.originStage !== 'WithTechnicalOfficer')))
      .catch(() => {});
    fetchExtensions()
      .then((all) => setTimelineExtensions(all.filter((e) => e.applicationId === id)))
      .catch(() => {});
  }, [id]);
  ...
  return (
    <div ref={printRef}>
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: '16px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        {activeTab === 2 && <TabQueries app={app} extensions={timelineExtensions} onResponded={() => fetchApplication(app.id).then(setApp)} />}
        {activeTab === 3 && (
  ...
  <div style="margin-top:32px;font-size:10px;color:#888;text-align:right">Issued on: ${td?.recordedAt ? new Date(td.recordedAt as string).toLocaleDateString('en-IN',{day:'2-digit',month:'long',year:'numeric'}) : '—'}</div>
              ...
              return (
                <div>
                      ...
                      {isApproved && (
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setMailSending(true);
                            try {
                              const { sentTo } = await sendCertificateEmail(app.id);
                          ...
                          style={{ background: '#059669', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 12, fontWeight: 600, cursor: mailSending ? 'not-allowed' : 'pointer', opacity: mailSending ? 0.7 : 1 }}>
                          {mailSending ? 'Sending…' : '📧 Send Certificate to Email'}
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  {/* PMS Report Submission */}
                  {isApproved && (td as Record<string, unknown>)?.withPms && (
                    <div style={{ marginTop: 20, border: `1px solid #FCD34D`, borderRadius: 10, overflow: 'hidden' }}>
                      <div style={{ background: '#FFFBEB', borderBottom: '1px solid #FCD34D', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontSize: 18 }}>🔬</span>
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#92400E' }}>Post Market Surveillance (PMS) Report</div>
                          <div style={{ fontSize: 11, color: '#B45309' }}>Your approval includes a PMS condition. Submit monitoring data or report below.</div>
                        </div>
                      </div>
                      <div style={{ padding: '16px 20px' }}>
                        <div style={{ marginBottom: 12 }}>
                          <label style={{ fontSize: 11, fontWeight: 600, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.4, display: 'block', marginBottom: 6 }}>Upload PMS Report</label>
                          <input
                            type="file"
                        ...
                        {pmsFile && (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const storedName = await uploadFile(pmsFile, app.id, 'pmsReport');
                                await submitPmsReport(app.id, storedName, pmsFile.name);
                                toast.success('PMS report submitted successfully');
                                setPmsFile(null);
                              } catch {
                            ...
                            style={{ background: '#D97706', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 12, fontWeight: 600, cursor: pmsUploading ? 'not-allowed' : 'pointer', opacity: pmsUploading ? 0.7 : 1 }}>
                            {pmsUploading ? 'Uploading…' : '⬆ Submit PMS Report'}
                          </button>
                        )}
                      </div>
                    </div>
        ...
        {activeTab === 4 && <TabTimeline app={app} queries={timelineQueries} extensions={timelineExtensions} />}
      </div>
    </div>
  );
}
