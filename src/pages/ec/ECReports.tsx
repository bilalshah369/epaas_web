...
import { fetchECAll } from '@/services/ec.service';
import type { Application } from '@/services/application.service';

type ReportSection = 'approved' | 'status' | 'track' | null;

const REPORTS = [
  { key: 'approved' as const, icon: '📈', title: 'EC Approved Applications Report', desc: 'Complete list of all EC-recommended approvals with dates and committee details.' },
  { key: 'status'   as const, icon: '📊', title: 'Applications Status Report',       desc: 'Stage-wise and ageing-wise status of all active applications reviewed by EC.' },
  { key: 'track'    as const, icon: '🔍', title: 'Track Application / EC Decision',  desc: 'Full lifecycle tracking for a specific application by reference or approval number.' },
];
...
export default function ECReports() {
  ...
  const load = useCallback(async () => {
    if (active === 'approved' || active === 'status') {
      setLoading(true);
      try {
        const data = await fetchECAll();
        setApps(data);
        setApprFiltered(data.filter((a) => ['Approved', 'Closed', 'WithNodalPointB'].includes(a.stage)));
  ...
  function applyApprFilter() {
    let r = apps.filter((a) => ['Approved', 'Closed', 'WithNodalPointB'].includes(a.stage));
  ...
  function resetApprFilter() {
    ...
    setApprFiltered(apps.filter((a) => ['Approved', 'Closed', 'WithNodalPointB'].includes(a.stage)));
    setApprApplied(false);
  }
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, fontFamily: "'Libre Baskerville',Georgia,serif", margin: 0 }}>EC Approved Applications</h3>
        </div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={lStyle}>Reference No.</label>
              <input value={apprRef} onChange={(e) => setApprRef(e.target.value)} placeholder="EPAAS-…" style={{ ...iStyle, minWidth: 140 }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={lStyle}>Company</label>
              <input value={apprCompany} onChange={(e) => setApprCompany(e.target.value)} placeholder="Search…" style={{ ...iStyle, minWidth: 140 }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={lStyle}>Product</label>
              <input value={apprProduct} onChange={(e) => setApprProduct(e.target.value)} placeholder="Keyword…" style={{ ...iStyle, minWidth: 140 }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={lStyle}>Category</label>
              <select value={apprCat} onChange={(e) => setApprCat(e.target.value)} style={{ ...iStyle, minWidth: 130, cursor: 'pointer' }}>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              {apprFiltered.length}{apprApplied ? ` of ${apps.filter((a) => ['Approved','Closed','WithNodalPointB'].includes(a.stage)).length}` : ''} records
            </span>
            <button onClick={() => exportCSV(apprFiltered, 'ec_approved_applications')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{['Sr. No.', 'Reference No.', 'Company / Org.', 'App. Type', 'Product Name', 'EC Decision', 'EC Date', 'Current Stage'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {apprFiltered.map((a, i) => (
                  <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }} onClick={() => navigate(`/ec/dockets/${a.id}`)}>
                    ...
                    <td style={S.td}><span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: COLORS.successLight, color: COLORS.success }}>Recommended Approval</span></td>
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <button onClick={() => exportCSV(allFiltered, 'ec_status_report')} style={{ marginLeft: 'auto', background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
        </div>
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{['Sr. No.', 'Application No.', 'Company / Applicant', 'Product', 'Date of Receipt', 'EC Status', 'Final Status'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {loading && <tr><td colSpan={7} style={{ ...S.td, textAlign: 'center', color: COLORS.textMuted, padding: 32 }}>Loading…</td></tr>}
                {allFiltered.map((a, i) => {
                  const isApproved = ['Approved', 'Closed', 'WithNodalPointB'].includes(a.stage);
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }} onClick={() => navigate(`/ec/dockets/${a.id}`)}>
                      ...
                      <td style={S.td}><span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: bg, color: fg }}>{isApproved ? 'Recommended' : isRejected ? 'Rejected' : 'Pending'}</span></td>
  ...
  if (active === 'track') {
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          <button onClick={() => setActive(null)} style={{ ...iStyle, color: COLORS.primary, fontWeight: 600, cursor: 'pointer' }}>← Back</button>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, fontFamily: "'Libre Baskerville',Georgia,serif", margin: 0 }}>Track Application / EC Decision</h3>
        </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
              ...
              <input value={track} onChange={(e) => setTrack(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter' && track.trim()) navigate(`/ec/dockets`); }}
                placeholder="EPAAS-…" style={{ ...iStyle, fontSize: 13, padding: '8px 12px' }} />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => track.trim() && navigate('/ec/dockets')} style={{ padding: '8px 22px', background: COLORS.primary, color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Search</button>
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={S.pageDesc}>Standard reports for EC monitoring and compliance.</div>
      </div>
