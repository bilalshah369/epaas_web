...
import { fetchECPending, fetchECAll } from '@/services/ec.service';
...
function ScreenHeading({ title }: { title: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, paddingBottom: 10, borderBottom: `1px solid ${COLORS.border}` }}>
      <div>
        <div style={{ fontSize: 9, fontWeight: 700, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.7, marginBottom: 2 }}>Expert Committee</div>
...
const NOTIFICATIONS = [
  { id: 1, title: 'New application for EC review — EPAAS-2025-007', desc: 'Mother Dairy NSF application forwarded from Technical Officer. Awaiting EC evaluation.', time: '1h ago', read: false },
  { id: 2, title: 'Docket prepared — EPAAS-2025-002', desc: 'Nestlé India Ltd. dossier is ready for committee review.', time: '3h ago', read: false },
  { id: 3, title: 'EC Meeting scheduled — EC-2026-07.15', desc: 'Next EC meeting is on 15 Jul 2026. Agenda has 8 applications.', time: '1d ago', read: true },
  { id: 4, title: 'Reminder: EPAAS-2025-003 — 5 days remaining', desc: 'Dabur Chyawanprash application review deadline approaching.', time: '2d ago', read: false },
];
...
export default function ECDashboard() {
  ...
  useEffect(() => {
    fetchECPending().then((data) => { setApps(data); setPendingDisplay(data); }).finally(() => setLoading(false));
    fetchECAll().then(setAllApps);
  }, []);
  ...
  const DASH_CARDS = [
    ...
    { key: 'rejected',     icon: '❌', label: 'Applications Rejected',            count: rejectedCount,  color: COLORS.danger  },
    { key: 'pms',          icon: '📋', label: 'Applications Approved with PMS',   count: null,           color: COLORS.info    },
    { key: 'withdrawn',    icon: '🔄', label: 'Applications Withdrawn / Closed',  count: withdrawnCount, color: COLORS.warning },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.6 }}>Expert Committee Work Queue</span>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button onClick={() => exportCSV(apps, 'ec_pending_dockets')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
        ...
        {loading ? (
          ...
          <div style={{ padding: '48px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, color: COLORS.textMuted }}>No applications pending EC review.</div>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['App. No.', 'Company', 'Product', 'App. Type', 'Food Category', 'Pending With', 'Received', 'Days Elapsed', 'Status', 'Action'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {apps.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg }}>
                      ...
                      <td style={{ ...S.td, fontSize: 11, color: COLORS.primary, fontWeight: 600 }}>Expert Committee</td>
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.6 }}>
            EC Case Dockets — Pending Review ({pendingDisplay.length}{filterApplied && pendingDisplay.length !== apps.length ? ` of ${apps.length}` : ''} records)
          </span>
          <button onClick={() => exportCSV(pendingDisplay, 'ec_pending_dockets')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>{['Sr. No.', 'App. Ref. No.', 'App. Type', 'Food Category', 'Company / Org.', 'Product Applied For', 'Forwarded By TO On', 'Days Elapsed', 'EC Status', 'Action'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ ...S.td, textAlign: 'center', padding: '40px 0', color: COLORS.textMuted }}>Loading…</td></tr>
              ) : pendingDisplay.length === 0 ? (
                <tr><td colSpan={10} style={{ ...S.td, textAlign: 'center', padding: '48px 0', color: COLORS.textMuted }}>{filterApplied ? 'No matching applications. Try clearing the filters.' : 'No applications pending EC review.'}</td></tr>
