...
const STAGE_OPTIONS: Record<string, string> = {
  ...
  'Forwarded to Nodal Point B': 'WithNodalPointB',
  'Approved': 'Approved',
  'Rejected': 'Rejected',
...
const SUMMARY_LABELS = [
  ...
  { label: 'Withdrawn / Closed',          fn: (a: Application) => a.stage === 'Withdrawn' },
];

export default function ECSearchConsole() {
  ...
  const doSearch = useCallback(async () => {
    ...
    try {
      ...
      if (decFrom)         r = r.filter((a) => ['Approved', 'Closed', 'Rejected', 'WithNodalPointB'].includes(a.stage) && new Date(a.updatedAt) >= new Date(decFrom));
      if (decTo)           r = r.filter((a) => ['Approved', 'Closed', 'Rejected', 'WithNodalPointB'].includes(a.stage) && new Date(a.updatedAt) <= new Date(decTo + 'T23:59:59'));

      setResults(r);
      setSearched(true);
    } finally { setLoading(false); }
  }, [company, stageKey, refNo, subFrom, subTo, decFrom, decTo]);

  function handleReset() {
    setCompany(''); setStageKey('All Stages'); setRefNo('');
    setSubFrom(''); setSubTo(''); setDecFrom(''); setDecTo('');
    ...
    <div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
          <div>
            <label style={lStyle}>Company / Organization</label>
            <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Search company…" style={iStyle} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={refNo} onChange={(e) => setRefNo(e.target.value)} placeholder="EPAAS-…" style={iStyle} />
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 14, marginBottom: 18 }}>
          ...
          <div><label style={lStyle}>EC Decision From</label><input type="date" value={decFrom} onChange={(e) => setDecFrom(e.target.value)} style={iStyle} /></div>
          <div><label style={lStyle}>EC Decision To</label>  <input type="date" value={decTo}   onChange={(e) => setDecTo(e.target.value)}   style={iStyle} /></div>
        </div>
      ...
      {searched && (
        <div style={{ background: '#fff', border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={() => exportCSV(results, 'ec_search_results')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['Sr. No.', 'Application No.', 'Company / Applicant', 'Product', 'App. Type', 'Food Category', 'Received On', 'Pending With', 'EC Decision', 'Final Status'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                ...
                {results.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }}
                      ...
                      <td style={S.td}>{isApproved ? '✅ Approved' : isRejected ? '❌ Rejected' : '—'}</td>
