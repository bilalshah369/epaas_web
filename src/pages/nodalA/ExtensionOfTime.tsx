...
// Wired to real API: fetchNodalAExtensionRequests().
import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { COLORS, S } from '@/utils/colors';
import { fetchNodalAExtensionRequests } from '@/services/officer.service';
...
export default function ExtensionOfTime() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setRequests(await fetchNodalAExtensionRequests()); } finally { setLoading(false); }
  }, []);

  useEffect(() => { load(); }, [load]);

  const completed = (r: ExtensionRecord) => r.status === 'Approved' || r.status === 'Rejected';
  const visible   = statusFilter === 'Pending'
    ? requests.filter((r) => r.status === 'Pending')
    : requests.filter((r) => completed(r));
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                {['App. Ref. No.', 'Company / Org.', 'Product', 'Product Category', 'Application Date', 'Requested Date', 'Status', 'Applicant Remarks', 'Documents', 'History', 'Action'].map((h) => (
              ...
              {visible.map((r, i) => {
                ...
                return (
                  <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg }}>
                    ...
                    <td style={S.td}>
                      <span title={r.justification} style={{ color: COLORS.textMuted, cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setRemarksModal(r.justification)}>View</span>
                    </td>
                    <td style={S.td}>
                      <span style={{ color: COLORS.primary, cursor: 'pointer', fontWeight: 600 }}>📎 Docs</span>
                    </td>
                    <td style={S.td}>
                      <span style={{ color: COLORS.primary, cursor: 'pointer', fontWeight: 600 }}>📋 History</span>
                    </td>
                    <td style={{ ...S.td, whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button onClick={() => navigate(`/nodal/scrutiny/${r.application.id}`)} style={{ padding: '4px 10px', background: 'transparent', color: COLORS.primary, border: `1px solid ${COLORS.primary}`, borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>View</button>
                        {r.status === 'Pending' ? (
                          <>
