...
const STAGE_PENDING_WITH: Record<string, string> = {
  ...
  ReviewPending: 'Chairperson', ReviewDisposed: '—',
...
const STAGE_OPTIONS: Record<string, string> = {
  ...
  'Review Disposed':         'ReviewDisposed',
  'Approved':                'Approved',
  'Rejected':                'Rejected',
};
...
const SUMMARY_LABELS = [
  ...
  { label: 'Reviews on File',        fn: (a: Application) => ['ReviewPending', 'ReviewDisposed'].includes(a.stage) },
];

export default function ChairpersonSearchConsole() {
      ...
      {searched && (
        <div style={{ background: '#fff', border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={() => exportCSV(results, 'chairperson_search_results')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
                ...
                {results.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }}
                      onClick={() => navigate(`/chairperson/reviews/${a.id}`)}>
