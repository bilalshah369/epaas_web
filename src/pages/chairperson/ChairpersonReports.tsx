...
import { fetchChairpersonAll } from '@/services/chairperson.service';
import type { Application } from '@/services/application.service';

type ReportSection = 'approved' | 'status' | 'track' | null;

const REPORTS = [
  { key: 'approved' as const, icon: '📈', title: 'Chairperson Approved Applications Report', desc: 'Complete list of all Chairperson-approved applications with dates.' },
  { key: 'status'   as const, icon: '📊', title: 'Applications Status Report',               desc: 'Stage-wise and ageing status of all applications handled by the Chairperson.' },
  { key: 'track'    as const, icon: '🔍', title: 'Track Application / Decision',             desc: 'Full lifecycle tracking for a specific application by reference number.' },
];
...
export default function ChairpersonReports() {
  ...
  const isApprovedStage = (a: Application) => ['Approved', 'Closed'].includes(a.stage);

  const load = useCallback(async () => {
    if (active === 'approved' || active === 'status') {
      setLoading(true);
      try {
        const data = await fetchChairpersonAll();
        setApps(data);
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, fontFamily: "'Libre Baskerville',Georgia,serif", margin: 0 }}>Chairperson Approved Applications</h3>
        </div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.bg }}>
            ...
            <button onClick={() => exportCSV(apprFiltered, 'chairperson_approved_applications')} style={{ background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{['Sr.', 'Reference No.', 'Company', 'App. Type', 'Product', 'Decision', 'Date', 'Stage'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {apprFiltered.map((a, i) => (
                  <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }} onClick={() => navigate(`/chairperson/reviews/${a.id}`)}>
                    ...
                    <td style={S.td}><span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: COLORS.successLight, color: COLORS.success }}>Final Approval</span></td>
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <button onClick={() => exportCSV(allFiltered, 'chairperson_status_report')} style={{ marginLeft: 'auto', background: 'none', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>⬇ Export CSV</button>
        </div>
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{['Sr.', 'App. No.', 'Company', 'Product', 'Date of Receipt', 'Decision Status', 'Final Stage'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {allFiltered.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg, cursor: 'pointer' }} onClick={() => navigate(`/chairperson/reviews/${a.id}`)}>
                      ...
                      <td style={S.td}><span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: bg, color: fg }}>{isApproved ? 'Final Approval' : isRejected ? 'Rejected' : 'Pending'}</span></td>
  ...
  if (active === 'track') {
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          <button onClick={() => setActive(null)} style={{ ...iStyle, color: COLORS.primary, fontWeight: 600, cursor: 'pointer' }}>← Back</button>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: COLORS.text, fontFamily: "'Libre Baskerville',Georgia,serif", margin: 0 }}>Track Application / Decision</h3>
        </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
              ...
              <input value={track} onChange={(e) => setTrack(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter' && track.trim()) navigate('/chairperson/reviews'); }}
                placeholder="EPAAS-…" style={{ ...iStyle, fontSize: 13, padding: '8px 12px' }} />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => track.trim() && navigate('/chairperson/reviews')} style={{ padding: '8px 22px', background: COLORS.primary, color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Search</button>
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <div style={S.pageDesc}>Standard reports for Chairperson monitoring and decisions.</div>
      </div>
