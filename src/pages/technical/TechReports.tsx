// Technical Officer — Reports Summary (/technical/reports/summary)
...
const STATUS_COLS = ['Sr. No.', 'Application No.', 'Name and Address of Applicant', 'Name of Product', 'Date of Receipt of Application', 'E.C Number', 'E.C Status', 'Date of Receipt of Appeal', 'Date of Appellate Order', 'Date of Receipt of Review', 'Date of Review Order', 'Date of Issue of Form II', 'Final Status'];
...
export default function TechReports() {
  ...
  const load = useCallback(async () => {
    if (active === 'approved' || active === 'status') {
      setLoading(true);
      try { setApps(await fetchTechnicalAll()); } finally { setLoading(false); }
    }
  }, [active]);
      ...
      <div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
            {[{ label: 'Approval No.', placeholder: 'APPR-…' }, { label: 'Reference No.', placeholder: 'EPAAS-…' }, { label: 'Company / Organization', placeholder: 'Search…' }, { label: 'Product Name', placeholder: 'Keyword…' }].map((f) => (
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{['Sr. No.', 'Approval No.', 'Reference No.', 'Company / Org.', 'Category', 'Product Name', 'Status', 'Issue Date'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
    ...
    return (
      <div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr>{STATUS_COLS.map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
  ...
  if (active === 'track') {
    return (
      <div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 16, maxWidth: 480 }}>
