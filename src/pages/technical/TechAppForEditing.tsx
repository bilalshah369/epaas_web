// Technical Officer — App for Editing / Clarification (/technical/editing)
...
export default function TechAppForEditing() {
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>{['Sr. No.', 'App. Ref. No.', 'App. Type', 'Application Name', 'Company / Org.', 'Forwarded On', 'Action'].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={7} style={{ ...S.td, textAlign: 'center', color: COLORS.textMuted, padding: 32 }}>Loading…</td></tr>}
              {!loading && apps.length === 0 && <tr><td colSpan={7} style={{ ...S.td, textAlign: 'center', color: COLORS.textMuted, padding: 32 }}>No applications pending editing / clarification.</td></tr>}
              {apps.map((a, i) => (
                <tr key={a.id} style={{ background: i % 2 === 0 ? '#fff' : COLORS.bg }}>
                  ...
                  <td style={{ ...S.td, whiteSpace: 'nowrap' }}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <Btn label="View Remarks"        onClick={() => navigate(`/technical/assessment/${a.id}`)} />
