// Technical Officer — Appeal and Review (/technical/appeal-review)
import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { COLORS, S } from '@/utils/colors';
import { fetchTechnicalAppealReview } from '@/services/technical.service';
...
export default function TechAppealReview() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setRecords(await fetchTechnicalAppealReview()); } finally { setLoading(false); }
  }, []);
      ...
      <div style={{ background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '12px 16px', marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
        {FILTERS.map((f) => (
          <div key={f.label} style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 140, flex: '1 1 140px' }}>
            ...
            {f.type === 'date' ? <input type="date" style={{ border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '5px 8px', fontSize: 11 }} />
