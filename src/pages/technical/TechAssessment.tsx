...
function btn(variant: 'solid' | 'outline' | 'danger' | 'success' = 'solid', small = false): React.CSSProperties {
  const bg = variant === 'solid' ? COLORS.primary : variant === 'success' ? COLORS.success : 'transparent';
  return {
    padding: small ? '4px 10px' : '7px 14px', borderRadius: 5,
    fontSize: small ? 11 : 12, fontWeight: 600, cursor: 'pointer',
    border: variant === 'solid' || variant === 'success' ? 'none' : `1.5px solid ${variant === 'danger' ? COLORS.danger : COLORS.primary}`,
    background: bg,
    color: variant === 'solid' || variant === 'success' ? '#fff' : variant === 'danger' ? COLORS.danger : COLORS.primary,
  };
}
...
<style>
  ...
  async function handleSendQuery() {
    ...
    try {
      await fetch(`${API_BASE}/applications/${appId}/queries`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('epaas_token')}` },
        body: JSON.stringify({ text }),
      });
      toast.success('Query sent to Nodal Officer');
      setQuerySubject(''); setQueryBody('');
  ...
  */
  ...
  async function handleRecordDecision() {
    ...
    if (f2Decision === 'Approved' && !f2Conditions.trim()) { toast.error('Conditions for approval are required'); return; }
    if (f2Decision === 'Rejected' && !f2Reasons.trim()) { toast.error('Reasons for rejection are required'); return; }
    ...
    <div>
      ...
      <div style={{ marginBottom: 12, paddingBottom: 10, borderBottom: `1px solid ${COLORS.border}` }}>
        ...
        <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 3 }}>
          ...
          {fromEC && <span style={{ marginLeft: 8, background: '#E8F5E9', color: '#2E7D32', borderRadius: 4, padding: '1px 7px', fontSize: 10, fontWeight: 700 }}>EC REVIEWED</span>}
        </div>
      </div>

      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${COLORS.border}`, marginBottom: 14, flexWrap: 'wrap' }}>
        {visibleTabs.map((t) => (
          <div key={t} onClick={() => setActiveTab(t)}
            ...
            {t === 'decision' && (
              <span style={{ marginLeft: 5, background: '#2E7D32', color: '#fff', borderRadius: 8, fontSize: 9, padding: '1px 5px', fontWeight: 700 }}>NEW</span>
            )}
          </div>
        ))}
      </div>
            ...
            <>
              {(() => {
                ...
                return (
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 6 }}>{passCount}/{compRows.length} items compliant</div>
                    <div style={{ height: 6, background: COLORS.border, borderRadius: 3 }}>
                      <div style={{ height: '100%', borderRadius: 3, background: passCount === compRows.length ? COLORS.success : COLORS.primary, width: `${(passCount / compRows.length) * 100}%`, transition: 'width 0.3s' }} />
                    </div>
                  </div>
                );
              })()}
              {compRows.map((c, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: `1px solid ${COLORS.border}` }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: c.passed ? COLORS.successLight : COLORS.dangerLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>
                    {c.passed ? '✓' : '✗'}
                  </div>
                  <span style={{ fontSize: 13, color: c.passed ? COLORS.text : COLORS.danger }}>{c.label}</span>
                </div>
              ))}
            </>
          )}
                    ...
                    <div key={q.id} style={{ border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
                      <div style={{ background: COLORS.primaryLight, padding: '10px 14px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: COLORS.primary }}>Query #{i + 1} — raised by {q.askedBy?.username ?? 'Officer'}</span>
                      ...
                      {isTechQuery && !q.nodalForwardedAt && (
                        <div style={{ background: '#FFF7ED', padding: '8px 14px', borderTop: `1px solid ${COLORS.border}` }}>
                          <span style={{ fontSize: 11, color: '#B45309', fontWeight: 600 }}>⏳ Pending — Nodal Officer to forward to applicant</span>
                        </div>
                      )}
      ...
      {activeTab === 'recommendation' && (
        <div style={card}>
          ...
          <div style={{ marginBottom: 6, fontSize: 12, color: COLORS.textMuted, lineHeight: 1.6 }}>
            After technical review, forward this application to the Expert Committee for evaluation. Provide your assessment remarks below.
          </div>
          <div style={{ marginBottom: 18, marginTop: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 6 }}>Technical Remarks / Grounds *</label>
            <textarea rows={6} value={recRemarks} onChange={(e) => setRecRemarks(e.target.value)}
          ...
          <div style={{ marginTop: 14, background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '10px 14px', fontSize: 11, color: COLORS.textMuted, lineHeight: 1.6 }}>
            <strong style={{ color: COLORS.text }}>Stage transition:</strong> Application moves from <strong>WithTechnicalOfficer</strong> → <strong>WithExpertCommittee</strong> for full committee review.
          </div>
        </div>
      )}

      {/* ── Prepare Decision tab (only when EC has reviewed) ─────── */}
      {activeTab === 'decision' && fromEC && (() => {
        const ecRec = (app.toDecision as Record<string,unknown>)?.ecDecision === 'RecommendRejection' ? 'Reject' : 'Approve';
        const readField = (label: string, value: string) => (
          <div style={{ marginBottom: 0 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 3 }}>{label}</div>
            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text }}>{value || '—'}</div>
          </div>
        );
        const sectionLabel = (text: string, required = false) => (
          <label style={{ fontSize: 11, fontWeight: 700, color: COLORS.text, textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 8 }}>
        ...
        return (
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: ecRec === 'Approve' ? '#F0FDF4' : '#FFF1F2', border: `1px solid ${ecRec === 'Approve' ? '#86EFAC' : '#FECDD3'}`, borderRadius: 10, padding: '12px 18px', marginBottom: 16 }}>
              <div style={{ fontSize: 22 }}>{ecRec === 'Approve' ? '✅' : '⚠️'}</div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: ecRec === 'Approve' ? '#166534' : '#9F1239' }}>
                ...
                <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 12, color: COLORS.textMuted }}>
                      Final Decision: <strong style={{ color: f2Decision === 'Approved' ? COLORS.success : COLORS.danger, fontSize: 13 }}>{f2Decision}</strong>
                    </span>
                  </div>
                  <button
